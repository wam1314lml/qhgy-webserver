...
脚本已修复SDK错误原因被丢弃，Web已补“账户不存在”和密码错误变体识别，并保留游戏401及ACCOUNT_PASSWORD_LOGIN_FAILED。前端既有AddAccountModal会显示后端安全message，失败后留在登录表单、清空密码/票据、恢复按钮，不需要改生产代码。新增组件回归覆盖HTTP200业务失败/HTTP401两条路径，Axios回归确认这些错误不退出网页登录，网站自身令牌失效仍正常退出。专项Vue编译及执行通过，未真实登录或绑定；此次没有改页面/样式，不重新构建或发布前端产物。
